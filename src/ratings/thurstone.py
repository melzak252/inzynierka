...
class ThurstoneRating(RatingSystem):
    def __init__(self, beta: float = 25.0 / 2.0, tau: float = 25.0 / 300.0, mu: float = 25.0, sigma: float = 25.0 / 3.0):
        self.tm_model = ThurstoneMostellerFull(beta=beta, tau=tau, mu=mu, sigma=sigma)
        self.team_ratings = defaultdict(lambda: self.tm_model.rating())
        self.player_ratings = defaultdict(lambda: self.tm_model.rating())
    ...
    def predict_team_win_prob(self, t1: str, t2: str) -> float:
        ...
        return self.tm_model.predict_win([[r1], [r2]])[0]
    ...
    def update_team(self, t1: str, t2: str, score_1: int, score_2: int) -> None:
        ...
        if score_1 > score_2:
            r1_new, r2_new = self.tm_model.rate([[r1], [r2]])
        elif score_2 > score_1:
            r2_new, r1_new = self.tm_model.rate([[r2], [r1]])
        else:
            r1_new, r2_new = self.tm_model.rate([[r1], [r2]], ranks=[0.0, 0.0])
