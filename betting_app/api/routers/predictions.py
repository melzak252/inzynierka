"""Router: /api/predictions — EV+ signals and model predictions overview."""
...
def list_predictions(
    ...
    min_books: int = Query(1, description="Minimum number of bookmakers for a match"),
    limit: int = Query(50, le=200),
    db: Connection = Depends(get_db),
):
    """Return EV+ signals from model_ev_signals joined with match metadata."""
    rows = query_df(
        db,
        """
        SELECT
